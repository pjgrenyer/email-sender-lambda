...
export const processMessage = async (message: Message) => {
    ...
    try {
        await recordEmail(toAddresses, ccAddresses, bccAddresses, uniqueId, message.subject, message.body?.html, message.body?.templateId, message.body?.data);
